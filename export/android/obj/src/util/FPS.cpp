...
{
HX_STACK_FRAME("util.FPS","new",0xd922c8cf,"util.FPS.new","util/FPS.hx",9,0x942bfc42)
HX_STACK_THIS(this)
...
Void FPS_obj::update( ){
{
		HX_STACK_FRAME("util.FPS","update",0xd9403cfa,"util.FPS.update","util/FPS.hx",30,0x942bfc42)
		HX_STACK_THIS(this)
		HX_STACK_LINE(31)
		...
		if ((tmp11)){
			...
			HX_STACK_LINE(42)
			::String tmp14 = (tmp13 + HX_HCSTRING("  MEM: ","\xdb","\x67","\xcb","\x81"));		HX_STACK_VAR(tmp14,"tmp14");
			HX_STACK_LINE(42)
			Float tmp15 = mem;		HX_STACK_VAR(tmp15,"tmp15");
			HX_STACK_LINE(42)
			::String tmp16 = (tmp14 + tmp15);		HX_STACK_VAR(tmp16,"tmp16");
			HX_STACK_LINE(42)
			::String tmp17 = (tmp16 + HX_HCSTRING(" MB","\x75","\x8b","\x18","\x00"));		HX_STACK_VAR(tmp17,"tmp17");
			HX_STACK_LINE(42)
			this->set_text(tmp17);
		}
		HX_STACK_LINE(44)
...
void FPS_obj::__register()
{
	...
	__mClass->mName = HX_HCSTRING("util.FPS","\x5d","\xb5","\x24","\x45");
